...
* * 5. `condition` *(int)* - состояние мода *(`0` - загружен, `1` - можно запрашивать, не до конца провалидирован, `2` - скачивается)*.
* * 6. `downloads` *(int)* - количество загрузок данного мода.
* * Доступно при `description=True`:
* * 7. `description` *(str)* - описание мода.
* * Доступно при `short_description=True`:
* * 8. `short_description` *(str)* - короткое описание мода.
* * Доступно при `dates=True`:
* * 9. `date_creation` *(`YYYY-MM-DD HH:MI:SS`)* - дата появления в первоисточнике.
* * 10. `date_update` *(`YYYY-MM-DD HH:MI:SS`)* - дата обновления в первоисточнике.
...
2. `description` *(`bool`)* - отправлять ли полное описание мода в ответе. По умолчанию `False`.
3. `dates` *(`bool`)* - отправлять ли дату последнего обновления и дату создания в ответе. По умолчанию `False`.
...
* * 2. `size` *(int)* - размер мода в байтах.
* * 3. `condition` *(int)* - состояние мода *(`0` - загружен, `1` - можно запрашивать, не до конца провалидирован, `2` - скачивается, `3` - в очереди на скачивание)*.
* * 4. `source` *(str)* - первоисточник.
* * 5. `downloads` *(int)* - количество загрузок данного мода.
* * Доступно при `description=True`:
* * 6. `description` *(str)* - описание мода.
* * Доступно при `dates=True`:
* * 7. `date_creation` *(`YYYY-MM-DD HH:MI:SS`)* - дата появления в первоисточнике.
* * 8. `date_update` *(`YYYY-MM-DD HH:MI:SS`)* - дата обновления в первоисточнике.
